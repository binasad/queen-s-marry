...
class JazzCashController {
  initiatePayment = async (req, res) => {
    try {
      ...
      // Persist either the cart payload or the legacy single-service shape into
      // the JSONB metadata column. _createAppointmentFromJazzCash reads this on
      // the return callback to materialise the appointment(s).
      ...
      if (normalizedCart) {
        metadataObj.cartItems = normalizedCart;
      } else {
  ...
  async _createAppointmentFromJazzCash(txnRefNo) {
    ...
    // Idempotency: if any appointment for this txnRefNo already exists, the
    // cart has already been materialised — skip a re-run on webhook retry.
    const existing = await query(
      'SELECT id FROM appointments WHERE payment_intent_id = $1',
      [txnRefNo]
    );
    if (existing.rows.length > 0) {
      console.log('✅ JazzCash: appointment(s) already created for', txnRefNo);
      return;
    }
    ...
    if (!userId) {
      console.error('❌ JazzCash: missing userId metadata', metadata);
      return;
    }

    // Prefer cart payload; fall back to the legacy single-service shape.
    let items = Array.isArray(metadata.cartItems) && metadata.cartItems.length > 0
      ? metadata.cartItems
      : null;
    if (!items) {
