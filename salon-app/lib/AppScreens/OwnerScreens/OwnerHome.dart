...
class _OwnerHomeState extends State<OwnerHome> {
  ...
  Future<void> loadUserData() async {
    // First, try to load from AuthProvider (fast, no API call)
    final authProvider = provider_package.Provider.of<app_auth.AuthProvider>(context, listen: false);
      ...
      if (userName != null && userName.isNotEmpty && userName != 'User') {
        ...
        if (userData['role'] != null) {
          if (userData['role'] is Map) {
            roleName = userData['role']?['name']?.toString();
          } else {
            roleName = userData['role']?.toString();
          }
        }
        ...
        if (mounted) {
          setState(() {
            ...
            profile = userData['profileImage']?.toString() ?? 
    ...
    // Only try API call if we don't have a name yet
    // Skip API call entirely if we already have the name to avoid 401 errors
    if (hasName) {
      return; // We already have the name, no need to call API
    }

    // Try to fetch from API only if we don't have name from AuthProvider
    try {
      ...
      if (user != null && mounted) {
        ...
        if (userName != null && userName.isNotEmpty) {
          final userRole = user['role'] as Map<String, dynamic>?;
          
          setState(() {
            name = userName;
            email = user['email']?.toString() ?? '';
            profile = user['profile_image_url']?.toString() ?? '';
            role = userRole?['name']?.toString() ?? '';
            switchUser = '';
          });
        }
      }
    } catch (e) {
      // Silently ignore API errors - the 401 is expected if token is invalid
      // We'll use AuthProvider data or show "User" as fallback
    }
  }
